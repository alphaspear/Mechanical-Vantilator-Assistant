<?php
    ...
    else
    {
        ...
        $id = $_SESSION['id'];
        $agecheck=mysqli_query($con,"SELECT age FROM patient_details WHERE email='$email'");
    ...
    if(@$_GET['q']== 'addData') 
    {
      ...
      $q3=mysqli_query($con,"INSERT INTO patient_data VALUES  ('$id','$hr','$rr','$spo2','$fio2','$peep','$pao2', NOW())");
...
<?php   
        $tocheck=0;
        $count=0;
        $q=mysqli_query($con,"SELECT * FROM patient_data WHERE id='$id' ORDER BY 'time' " );
    ...
    <?php
        if($count>=3)
        {
            $hrcheck3=mysqli_query($con,"SELECT hr FROM patient_data ORDER BY time DESC LIMIT 3");
            $hr3 = array(0,0,0);
            $i=0;
            ...
            $rrcheck3=mysqli_query($con,"SELECT rr FROM patient_data ORDER BY time DESC LIMIT 3");
            $rr3 = array("0","0","0");
            $i=0;
            ...
            for($i=0;$i<=2;$i++)
            {
                if(($age==1)||($age==2))
                {
	                if(($hr3[$i]>=80)&&($hr3[$i]<=120))
	                {
	                    if(($rr3[$i]>=22)&&($rr3[$i]<=37))
	                    {
		                $flag++;
	                    }
	                }
                ...
                {
	                if(($hr3[$i]>=65)&&($hr3[$i]<=100))
	                {
    	                if(($rr3[$i]>=20)&&($rr3[$i]<=28))
	                    {
		                $flag++;
	                    }
	                }
                ...
                {
	                if(($hr3[$i]>=58)&&($hr3[$i]<=90))
	                {
	                    if(($rr3[$i]>=18)&&($rr3[$i]<=25))
	                    {
		                $flag++;
	                    }
	                }
                ...
                {
	                if(($hr3[$i]>=50)&&($hr3[$i]<=100))
	                {
	                    if(($rr3[$i]>=12)&&($rr3[$i]<=20))
	                    {
		                    $flag++;
	                    }
	                }
...
<script>
  ...
  <script>
        ...
        var tocheck =<?php echo $tocheck; ?>;
        if(count<3)
        {
            document.getElementById("output").innerHTML = "Insufficient Data for prediction";
            return;
        }
        if(count>2 && tocheck==0)
        {
            document.getElementById("output").innerHTML = "Patient is not ready to wean1";
            return;
        }
        if(count>2 && tocheck==1)
        {
            <?php
            // data to be sent to the POST request
            $qag=mysqli_query($con,"SELECT * FROM patient_data WHERE id=$id ORDER BY time DESC LIMIT 1" );
        while($row=mysqli_fetch_array($qag) )
                        {
                            $spo2=$row['spo2'];
                            $fio2=$row['fio2'];
                            $peep=$row['peep'];
                            $pao2=$row['pao2'];
                            echo'
            let _data = {spo2:'.$spo2.',fio2:'.$fio2.',peep:'.$peep.',pao2:'.$pao2.'}
                            ';                     
                        }
            ?>
            fetch('http://192.168.29.164:3138/analize', {
            method: "POST",
